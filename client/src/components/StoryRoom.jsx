...
export default function StoryRoom({ lobby, socketId }) {
  ...
  const effectiveLimit  = jokerActive && jokerAvailable ? lobby.wordLimit * 2 : lobby.wordLimit
  ...
  return (
    <div className="story-room">
      ...
      <div
        className={`bg-layer bg-current ${bgFade ? 'fading-in' : ''}`}
      ...
      <div className="room-layout">
        <header className="room-header">
            ...
            {lobby.players.map(p => {
              ...
              return (
                <div
                  ...
                  style={{ '--pc': p.color }}
                  title={gone ? `${p.name} (disconnected)` : p.name}
                >
                  <span className="chip-emblem">{p.emblem}</span>
                  <span className="chip-name">{p.name}</span>
                  {p.jokerAvailable && !gone && <span className="chip-joker" title="Has Joker ready">🃏</span>}
          ...
          <div className="room-stats">
            <span title="Total turns taken">📖 {lobby.promptCount}</span>
            <span className={lobby.nextImageIn <= 1 ? 'stat-imminent' : ''} title="Turns until next scene painting">
        ...
        <main className="story-scroll" ref={scrollRef}>
          ...
          <div className="story-text">
            {lobby.story.map((seg, idx) => (
              <span
                key={idx}
                className={`story-segment ${seg.playerId === socketId ? 'mine' : ''} ${seg.usedJoker ? 'joker-seg' : ''}`}
                style={{ '--pc': seg.playerColor }}
                title={`${seg.playerName}${seg.usedJoker ? ' · 🃏 Joker turn' : ''} · ${fmtTime(seg.timestamp)}`}
              >
                {idx > 0 && ' '}
                {seg.usedJoker && <span className="joker-mark" title="Joker used">🃏</span>}
        ...
        <footer className="input-area">
          {isMyTurn ? (
            <div className={`input-panel active ${jokerActive ? 'joker-mode' : ''}`} style={{ '--pc': myPlayer?.color }}>
              <div className="input-top-row">
                ...
                {jokerAvailable ? (
                  <button
                    className={`joker-btn ${jokerActive ? 'joker-btn-on' : ''}`}
                    onClick={() => setJokerActive(v => !v)}
                    title={jokerActive ? 'Cancel Joker' : `Use Joker — doubles limit to ${lobby.wordLimit * 2} words`}
                  >
                    🃏 {jokerActive ? 'Joker ON' : 'Use Joker'}
                  </button>
                ) : (
                  <div className="joker-cooldown">
                    🃏 recharges in {jokerRechargeIn} turn{jokerRechargeIn !== 1 ? 's' : ''}
                  </div>
                )}
              </div>
              {jokerActive && (
                <div className="joker-banner">
                  ✦ Joker active — limit doubled to {lobby.wordLimit * 2} words this turn
                </div>
              )}
              <div className="input-row">
                ...
                <button
                  ...
                  disabled={overLimit || wordCount === 0}
                  title="Submit (Enter)"
                >
                  {jokerActive ? '🃏' : '✦'}
                </button>
              ...
              <div className="input-meta">
                ...
                <span className="enter-hint">⏎ Enter to submit</span>
              </div>
            </div>
          ) : (
              ...
              {myPlayer && (
                <div className="your-joker-status">
                  {myPlayer.jokerAvailable
                    ? '🃏 Your Joker is ready'
                    : `🃏 Joker recharges in ${myPlayer.jokerRechargeIn} turn${myPlayer.jokerRechargeIn !== 1 ? 's' : ''}`}
                </div>
              )}
            </div>
          )}
